...
async function seed() {
  ...
  // LENDERS — real table now (was a hardcoded array in the route). Seeded
  // as 'illustrative'; flip a row to partnership_status='active' once a
  // real lending partner is signed — see src/routes/admin.js.
  const lendersSeed = [
    { name:'Agrifinance Partners', min_score:600, max_amount_ngn:5000000, rate_pa_pct:18, tenure_months:[3,6,12], contact:'loans@agrifinance.ng' },
    { name:'NIRSAL Microfinance Bank', min_score:500, max_amount_ngn:2000000, rate_pa_pct:21, tenure_months:[6,12,24], contact:'agri@nirsal.com' },
    { name:'Bank of Agriculture Nigeria', min_score:550, max_amount_ngn:10000000, rate_pa_pct:15, tenure_months:[12,24,36], contact:'loans@boanigeria.com' },
  ];
  for (const l of lendersSeed) {
    await query(
      `INSERT INTO lenders (name, min_score, max_amount_ngn, rate_pa_pct, tenure_months, contact, partnership_status)
       VALUES ($1,$2,$3,$4,$5,$6,'illustrative')
       ON CONFLICT (name) DO NOTHING`,
      [l.name, l.min_score, l.max_amount_ngn, l.rate_pa_pct, l.tenure_months, l.contact]
    );
  }
  console.log(`  ✓ ${lendersSeed.length} lenders seeded (illustrative — flip via admin API once signed)`);

  // EXPORT AGENTS — same treatment as lenders.
  const exportAgentsSeed = [
    { name:'Lagos Cocoa Export Ltd', crops:['Cocoa','Sesame'], states:['Lagos','Ogun'], contact:'export@lagoscocoa.ng', port:'Apapa' },
    { name:'Kano Agro Exports', crops:['Sesame','Soybean','Groundnut'], states:['Kano','Kaduna'], contact:'info@kanoexports.ng', port:'Kano Dry Port' },
    { name:'AfroCashew Nigeria', crops:['Cashew'], states:['Lagos','Ogun','Ondo'], contact:'trade@afrocashew.ng', port:'Tin Can Island' },
    { name:'Delta Palm Exports', crops:['Palm Oil'], states:['Delta','Rivers','Bayelsa'], contact:'export@deltapalmng.com', port:'Warri Port' },
    { name:'North Hibiscus Traders', crops:['Hibiscus','Sesame'], states:['Kano','Jigawa','Bauchi'], contact:'+2348099887766', port:'Kano Dry Port' },
  ];
  for (const a of exportAgentsSeed) {
    await query(
      `INSERT INTO export_agents (name, crops, states, contact, port, partner)
       VALUES ($1,$2,$3,$4,$5,false)
       ON CONFLICT (name) DO NOTHING`,
      [a.name, a.crops, a.states, a.contact, a.port]
    );
  }
  console.log(`  ✓ ${exportAgentsSeed.length} export agents seeded (illustrative — flip via admin API once signed)`);
