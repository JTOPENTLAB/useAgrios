...
- **Weekly cohort digest email** — `/app/backend/services/cohort_digest.py` builds + sends a Monday-9am-WAT admin email with: subject line rotation based on the week's signals, headline, last-week stat table with WoW deltas (signups ↑/↓ / depositors / first investors / volume), per-milestone retention with delta vs the prior eligible cohort ("W+1: 32% ↑ 8 pts"), full 8-week cohort matrix, and auto-generated "What to watch next" action items. Reuses the existing `services.digest.send_email` sender (Resend/SendGrid/mock fallback).
- **Endpoints** (all admin-only) `GET /api/admin/cohort-digest/preview`, `POST /api/admin/cohort-digest/send-me-now` (single admin), `POST /api/admin/cohort-digest/trigger` (blast to all admins), `GET /api/admin/cohort-digest/log`.
- **Scheduler** — `cohort_digest.scheduler_loop(db)` registered at startup. Hourly tick; fires once per Monday at `COHORT_DIGEST_HOUR_UTC` (default 8 UTC = 9am WAT). Idempotent via `system.cohort_digest_last_run`. Controlled by `ENABLE_CRON` flag.
- **Frontend action button** — `<WeeklyDigestButton/>` next to the cohort card title (`cohort-digest-send-btn`). One click → `POST /admin/cohort-digest/send-me-now` → in-place emerald "Sent ✓" confirmation + status message showing provider (mock when `RESEND_API_KEY` unset).
- **Tests** `/app/backend/tests/test_phase_o.py` — 12/12 pass (cohort auth + shape + clamp + eligibility, pre-launch regression smoke, digest preview/send-me-now/trigger/log happy paths, 403 guards for investor, idempotency confirmed via log row count).
