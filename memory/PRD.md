...
- **Chat delivery (Slack + WhatsApp)** — same digest also pushed to configured chat channels: `send_slack_webhook()` posts Slack Block Kit payload to `SLACK_WEBHOOK_URL` (Slack-compatible, also works for Discord/Google Chat/Zapier→anything), and `send_whatsapp_cloud()` posts a one-liner via Meta WhatsApp Cloud API (`WHATSAPP_PHONE_NUMBER_ID` + `WHATSAPP_ACCESS_TOKEN` + `WHATSAPP_TO`). Both gracefully return `status='skipped'` with a clear error hint when env isn't configured — zero-downtime opt-in. Each send logged to `digest_log` with `provider='slack'|'whatsapp'` for audit.
- **Endpoints** (all admin-only) `GET /api/admin/cohort-digest/preview`, `POST /api/admin/cohort-digest/send-me-now` (email + chat channels), `POST /api/admin/cohort-digest/trigger` (blast to all admins + chat channels), `POST /api/admin/cohort-digest/test-webhooks` (chat-only dry run), `GET /api/admin/cohort-digest/log`. All non-admin callers get 403.
